...
class _MyPainterPageState extends State<MyPainterPage> {
  ...
  void initState() {
    ...
    eventBus.on<NextPicture>().listen((event) { setState(() {
      this._imgPath=event.imgPath;
  ...
  capturePng(index, time) async {
    try {
      ...
      setAnswer(index, imagePath: imageFile.path.toString(), imageName: 'capture.png');
      //保存图片到相册的方法
...
//          rotationPoints(orientation, size);
  ...
  Widget _getCustomPainter(){
    ...
    if(this._imgPath==""){
      ...
      return CustomPaint(
          ...
          child: RepaintBoundary(
              child: Container(
                child: Image.asset(_imgPath,width: setWidth(1100),),
              )
          )
      );
    }
  ...
  Widget bottomPencilWidthButton(int index) {
    ...
    double bottom = 10.0 + (48.0 - size) / 2;
